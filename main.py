...
CONFIG = {
    # 🔥 [新增开关] 测试模式
    # True  = 手动测试：不检查节假日(周末也能跑)，计算结果但不保存到 history.csv (防止数据污染)
    # False = 自动运行：检查节假日(非交易日不跑)，结果保存到 history.csv (正式记录)
    "IS_TEST_MODE": True, 
...
def is_trading_day():
    ...
    try:
        ...
        if today in trade_dates:
            print(f"✅ 今天 ({today}) 是交易日，准备启动...")
            return True
        else:
            print(f"🛑 今天 ({today}) 是法定节假日/休市日。")
            return False
    except:
        # 如果接口挂了，且不是周末，默认通过(避免漏跑)
        print("⚠️ 日历接口连接失败，仅依周末判断，强制执行。")
        return True
...
def process_stock_logic(df, stock_info):
    ...
    # 策略C: 机构控盘 (ADX放宽至20)
    if not signal_type and curr["close"] > curr["MA60"] and curr["CMF"] > 0.05 and curr["ADX"] > 20:
    ...
    if curr["OBV"] <= curr["OBV_MA10"]: risk_list.append("资金流出(需谨慎)") 
...
def calculate_score_and_details(row):
    ...
    if "流出" in str(row.get('OBV状态', '')):
        score -= 20; details.append("📉OBV流出-20")
...
def save_and_beautify(data_list):
    ...
    filename = f"严选_智能版_{mode_str}_{dt_str}.xlsx"
    
    if not data_list:
        pd.DataFrame([["无股入选 (请检查市场环境)"]]).to_excel(filename)
        print("⚠️ 结果为空，已生成空表格")
        return filename
    ...
    ws.cell(row=start_row, column=1, value="⚔️ 小白使用指南").font = cat_font
    start_row += 1
    indicators = [
        ("近3日涨跌", "🆕 股性透视镜：如 '+1% ➡ +3% ➡ +6%' 代表加速上涨，如果是 '+5% ➡ +2% ➡ -1%' 代表主力可能在出货。"),
        ("选股理由", "看这个就够了！告诉你为什么选它 (例如: 黄金坑+外资)。"),
        ("风险提示", "🔥 最重要！看到'业绩亏损'或'短期涨幅过大'要特别小心。"),
        ("操作建议", "🤖 傻瓜指令：告诉你该多少钱买，多少钱止损。"),
